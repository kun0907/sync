...
public class indexDataServiceImpl extends ActionBase {
	 ...
	 * @param fileInfos 
	 */
	public String norm_graph(Map<String, Object> map, List<FileInfo> fileInfos){
		
		log.info("........图形数据");
		
		Map<String, String> graph = new HashMap<String, String>();
		graph.put("包长分布图", packetLenData(map));
		graph.put("吞吐曲线图", hufPufFlow(map,fileInfos));
		graph.put("吞吐曲线图detail", hufPufFlowDetail(map,fileInfos));
	...
	public String packetLenData(Map<String, Object> map){
	    ...
	    for (int i = 0; i < filecheck.size(); i++) {
		    ...
		    Map<String, Object> tcpGraph = (Map<String, Object>) map.get(file);
			...
			while (plsKeys.hasNext()) {
				...
				List<String> arrs = toList(arrStr.substring(1, arrStr.length()-1).split(","));
				Iterator<String> lens = arrs.iterator();
				
				int len = 0;
				while (lens.hasNext()) {
					String lenStr = lens.next();
					if(lenStr.equals(""))
						continue;
			...
			li.add("101-1000");
			if(max>=1001)
				li.add("1001-"+String.valueOf(max));
			else
				li.add("1001-1001");
			...
			log.info(JSONArray.fromObject(pld));
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String hufPufFlow(Map<String, Object> map, List<FileInfo> fileInfos){		//吞吐曲线图
        ...
        for (FileInfo file : fileInfos) {
			if(Double.parseDouble(file.getAllKeepTimes())< 500)
				timeNum++;
		}
        int fiNum = filecheck.size();
        
        Iterator<String> files = filecheck.iterator();
        while (files.hasNext()) {

            Map<String, Object> tcpGraph = (Map<String, Object>) map.get(files.next());
            Map<String, Integer> flowInfo;
            if(timeNum == fiNum)
            	flowInfo = (Map<String, Integer>) tcpGraph.get("吞吐曲线图ms");
            else
            	flowInfo = (Map<String, Integer>) tcpGraph.get("吞吐曲线图");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String hufPufFlowDetail(Map<String, Object> map, List<FileInfo> fileInfos){		//吞吐曲线图
        ...
        while (files.hasNext()) {
        	...
        	String id = "cuse2Detail"+String.valueOf(i++); // 折线图堆叠
            ...
            list += "'" + file + "'";
            list_y0 += "{name:'" + file + "',type:'line',data:" + y.get(file) + ""
                    + ",markPoint: {data: [{type: 'max', name: '最大值'},{type: 'min', name: '最小值'}]}}";
            ...
            map0.put("title", file);
            map0.put("list", list);
            map0.put("x_str", x.toString());
            map0.put("list_y", list_y0);
            if(timeNum == fiNum)
            	map0.put("unit", "ms");
            else
            	map0.put("unit", "s");
            res.add(JSON.toJSONString(map0));
        }
        return JSON.toJSONString(res);
    }
    /**
    ...
    public String ipFlowData(Map<String, Object> map) { // IP流量分布图
        ...
        for (int i = 0; i < filecheck.size(); i++) {
            ...
            Map<String, Object> tcpGraph = (Map<String, Object>) map.get(fileName);
            ...
            list_y += "{name:'IP流量分布图',type:'bar',itemStyle:{normal:{label:{show:true,position:'top',}}},barWidth: '30',data:" + y.toString() + "}";
            list_y += "]";
    ...
    public String ipLinkData(Map<String, Object> map) { // 链路图
        ...
        for (int i = 0; i < filecheck.size(); i++) {
            ...
            String file = files.next();
            Map<String, Object> tcpGraph = (Map<String, Object>) map.get(file);
            Map<String, Object> https = (Map<String, Object>) tcpGraph.get("tcpInfos");
            Set<String> ips = https.keySet();
            BigDecimal endTsLast = new BigDecimal(0); 
            for (String ip : ips) {
            	
				 Map<String, Object> tcpIP = (Map<String, Object>) https.get(ip);
            ...
            for (String ip : ips) {
                if (ip.equals("tcpGraph") || ip.equals("无效IP") || ip.equals("terminalBrand") 
                 || ip.equals("terminalModel") || ip.equals("logFileData1") || ip.equals("logFileData9"))
                    continue;
                Map<String, Object> tcpIP = (Map<String, Object>) https.get(ip);
                for (int j = 0; j < tcpIP.size(); j++) {
                	...
                	if(close33Time.compareTo(zero)==1){
                		if( close33Time.compareTo(laseFinUpTime)==1)
                			FIN.add(close33Time.subtract(laseFinUpTime).multiply(ms).setScale(2, BigDecimal.ROUND_HALF_UP));
                		else if( close33Time.compareTo(fillData1Ts)==1)
                			FIN.add(close33Time.subtract(fillData1Ts).multiply(ms).setScale(2, BigDecimal.ROUND_HALF_UP));
                		else if( close33Time.compareTo(setup3Ts)==1)
                			FIN.add(close33Time.subtract(setup3Ts).multiply(ms).setScale(2, BigDecimal.ROUND_HALF_UP));
                		else if( close33Time.compareTo(startTs)==1)
                			FIN.add(close33Time.subtract(startTs).multiply(ms).setScale(2, BigDecimal.ROUND_HALF_UP));
                	}else
                		FIN.add(zero);
                		
                    if(close33Time.compareTo(zero)==1)
                    	Delay.add(endTs.subtract(close33Time).multiply(ms).setScale(2, BigDecimal.ROUND_HALF_UP));
                    else
                    	Delay.add(zero);
            ...
            Map<String, Object> maps = ids.startDesc(START,y,DNS,Interval,SYN,FIRST_DATA,Interactive,FIN,Delay);
