...
public class TCPStream extends Stream {
	...
	private Map<Integer, Integer> mapRate1 = new HashMap<Integer, Integer>(); //吞吐流量 ms
	...
	public void checkHandshake(double logEndTs) {
		if(packets.size() < 3) {
			return;
		} 
		...
		for(Packet packet:packets) {
			...
			// 吞吐曲线
			if(logEndTs<0.5){
				deltaTs = Integer.parseInt(dfAfter.format(new BigDecimal(p.getDeltaTs()*1000)));//  时间向下取整(ms)
				
				if(mapRate1.containsKey(deltaTs))
					mapRate1.put(deltaTs, mapRate1.get(deltaTs) + p.getEthLength());
				else
					mapRate1.put(deltaTs, p.getEthLength());
			}
	...
	public Map<Integer, Integer> getMapRate1() {
		return mapRate1;
	}
