...
public class ActionBase extends Global{
	...
	//判断session是否有效
	public Boolean effecTive(HttpServletRequest request,HttpServletResponse response) {

		Boolean userState = false;
		
		Object username = request.getSession().getAttribute("username");
		if(username!=null && username.equals("cmcc"))
			return true;
	...
	//判断session是否有效  地址请求
	public boolean effecTiveLink(HttpServletRequest request,HttpServletResponse response) throws IOException {
    	...
    	if(username==null && sessionid.equals("")){
    		...
    		map.put("session", "remote");
    		response.setContentType("text/html;charset=UTF-8");
    		out.println("<script>alert('用户已在其它地点登录');");
    		out.println("top.location.href='"+path+"'");
       		out.println("</script>");
       		out.close();
       		return false;  
	...
	public String ipInfo(String ip,String baiduKey) throws Exception{
		...
		if(!ipInfo.isEmpty()){
			...
			String sql = "INSERT IGNORE INTO SYS_IP (`IP`, `IPADDR`, `UPDATETIME`, `FORM`) VALUES ('"+ip+"', '"+ipInfo+"', '"+df.format(System.currentTimeMillis())+"', '"+form+"');";
			execute(sql);
		}
		return ipInfo;
	}
	 ...
	 *  创建目录
	 * @throws IOException */
	 public static void createDir(String destDirName, HttpServletResponse response) throws IOException {
	 	...
	 	String Slash = "/";
	 	if (!destDirName.endsWith(Slash)) {// 结尾是否以"/"结束
	 		destDirName = destDirName + Slash;
	 	}
	 	if (dir.mkdirs()) {// 创建目标目录
	 		log.info("创建目录成功！" + destDirName);
	 		return;
	 	} else {
	 		log.error("创建目录失败！"+destDirName);
			response.setContentType("text/html;charset=utf-8");
			PrintWriter out = response.getWriter();
			out.println("<script>alert('用户仓库创建失败！');");
			out.println("</script>");
			out.close();
	 	}
	 }
