...
public class AnalyzeController extends ActionBase {
	...
	public void export2ftl(HttpServletRequest request,HttpServletResponse response) throws Exception {
			...
			if(isCompare){//比较时
					...
					try {
							...
							String userAgent_jingpinFile="";
							
							if(fileInfoList.get(0).getFileName().equals(jingpinFileName)){
								baseFileName=fileInfoList.get(1).getFileName();
								userAgent_baseFile=fileInfoList.get(1).getUserAgent();
								userAgent_jingpinFile=fileInfoList.get(0).getUserAgent();
							}else{
								baseFileName=fileInfoList.get(0).getFileName();
								userAgent_baseFile=fileInfoList.get(0).getUserAgent();
								userAgent_jingpinFile=fileInfoList.get(1).getUserAgent();
							}
	...
	public static void main(String[] args) throws Exception {
					 //创建工作薄
			        WritableWorkbook workbook = Workbook.createWorkbook(new FileOutputStream(new File("D://xxx.xls")));
			        //创建新的一页
	...
	public void ExportExcel(HttpServletRequest request,HttpServletResponse response) throws Exception {
			...
			if(isCompare){//比较时
					...
					List<FileInfo> fileInfoList = (List<FileInfo>) data_map.get("fileInfos");//fileInfo即是baowen
					if(fileInfoList!=null&&fileInfoList.size()>0){//one  fileInfo.one sheet
							
							//创建工作薄
							String filePath="D://xxx.xls";
							WritableWorkbook workbook = Workbook.createWorkbook(new FileOutputStream(new File(filePath)));
							for (FileInfo fileInfo : fileInfoList) {
								  //创建新的一页
							        WritableSheet sheet = workbook.createSheet(fileInfo.getFileName(),0);
							        //创建要显示的内容,创建一个单元格，第一个参数为列坐标，第二个参数为行坐标，第三个参数为内容
							      /*  Label label = new Label(colNum,rowNum,"学校");
							        sheet.addCell(label);*/
							        //title
							        Label label = new Label(0, 0, "基本信息");
							        sheet.addCell(label);
							        
							        label = new Label(0, 1, "持续时间(ms)");
							        sheet.addCell(label);
							        
							        label = new Label(1, 1, "数据传输时间(ms)");
							        sheet.addCell(label);
							        
							        label = new Label(2, 1, "传输总包数(个)");
							        sheet.addCell(label);
							        
							        label = new Label(3, 1, "传输量最大IP及端口");
							        sheet.addCell(label);
							        
							        label = new Label(4, 1, "传输总字节(byte)");
							        sheet.addCell(label);
							        
							        label = new Label(5, 1, "总链路数(个)");
							        sheet.addCell(label);
							        
							        label = new Label(6, 1, "链路最多IP");
							        sheet.addCell(label);
							        
							        label = new Label(7, 1, "链路最多IP(链路数)");
							        sheet.addCell(label);
							        
							        //data
							        label = new Label(0, 2, fileInfo.getAllKeepTimes());
							        sheet.addCell(label);
							        
							        label = new Label(1, 2, fileInfo.getAllFileDataTimes());
							        sheet.addCell(label);
							        
							        label = new Label(2, 2, fileInfo.getPacketCount()+"");
							        sheet.addCell(label);
							        
							        label = new Label(3, 2, fileInfo.getMaxFlowIP()+"("+fileInfo.getMaxFlowPort()+")");
							        sheet.addCell(label);
							        
							        label = new Label(4, 2, fileInfo.getExchangeFlowCount()+"");
							        sheet.addCell(label);
							        
							        label = new Label(5, 2, fileInfo.getTcpCount()+"");
							        sheet.addCell(label);
							        
							        label = new Label(6, 2, fileInfo.getMaxTCPip());
							        sheet.addCell(label);
							        
							        label = new Label(7, 2, fileInfo.getIpTCPCounts()+"");
							        sheet.addCell(label);
...
//		sysInfo.setTerminalModel("");	
		...
		if(fileInfos!=null&&fileInfos.size()>0){
				  ...
				  for (FileInfo fi : fileInfos) {
					     userAgents.add(fi.getFileName()+"："+fi.getUserAgent());
				  }
				  sysInfo.setUserAgents(userAgents);
		}
