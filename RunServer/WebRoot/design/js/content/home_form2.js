...
function fileType(){
	myFunction();
	var filePaths  = $('input[name="file"]')[0].files;
	...
	if(files.length+filePaths.length > 10){
		alert("最多允许存储10个文件，请清理后上传");
		return false;
	}
	for( var i=0;i<filePaths.length; i++ ){  
		
		if(filePaths[i].name.split(".")[1].toLowerCase() != "pcap"){
			alert("请选择pcap文件");
			return false;
		}
		
		if(filePaths[i].name.length >23){
			alert("文件名长度不能超过23个字");
			return false;
		}
		
		var upFileNameRule = filePaths[i].name.split("_");
		if(upFileNameRule.length > 4 || upFileNameRule.length < 2){
			alert("请校验文件名格式是否正确");
			return false;
		}
		var curFile;
		for(var j=0;j<files.length;j++){
			curFile = files[j].id;
			if(filePaths[i].name == curFile.substr(curFile.indexOf("@")+1)){
				alert("系统已存在相同名称文件");
				return false;
			}
		}
...
var filecheck = null;
function root(){//可以理解为："分析+导出到html",而"生成pdf"="分析+导出到pdf"
	myFunction();
...
function nums(this_){//点击文件前的复选框触发此函数
	...
	if(this_.checked){//选中
		 var exist=false;
		 for(var k=0; k<fileNameArrByCheckedOrder.length; k++) {
			    if(fileNameArrByCheckedOrder[k] == currFileName) {
			    		exist=true;
			    		break;
			    }
		  }
		 ...
		 for(var x=0; x<fileNameArrByCheckedOrder.length; x++) {
			    if(fileNameArrByCheckedOrder[x] == currFileName) {
			    		fileNameArrByCheckedOrder.splice(x, 1);
			    		break;
			    }
		  }
	}
	...
	if(jingpinFileName!=null)
		jingpinFileName = jingpinFileName.substring(0,jingpinFileName.length-5);
