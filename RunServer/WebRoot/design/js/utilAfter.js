...
function linkChart1(title,id,y,list_y,topLeg,topGr) {
	...
	id = {
        ...
        legend: {top:topLeg,data:['DNS Lookup(ms)', 'Interval(ms)', 'SYN(ms)','Time to First Byte(ms)','Interactive(ms)','FIN(ms)','Delay(ms)'],selectedMode:false,x:'right'},
...
function linkChart2(title,id,y,list_y,topLeg,topGr) {
	...
	/*	position: function (point, params, dom) {
        ...
        legend: {top:topLeg,data:['DNS Lookup(ms)', 'Interval(ms)', 'SYN(ms)','Time to First Byte(ms)','Interactive(ms)','FIN(ms)','Delay(ms)'],selectedMode:false},
        grid: {top:topGr,left: '0%',right: '3%',bottom: '0%',containLabel: true},calculable : true,
        xAxis : [{type : 'value',axisLabel:{formatter:function(val) {return val+"ms";}}}],
        yAxis : {show:false,type : 'category',data :eval(y),axisLabel:{formatter:function(val) {return val;},interval:0}},
        series : eval(list_y),color:['#008B8B','#8A2BE2','#CDCD00','#00FF00','#00BFFF','#CD0000','#7A8B8B'],barGap:'1%'
...
function linkChart3(title,id,y,list_y,topLeg,topGr) {
	...
	id = {
        ...
        legend: {top:topLeg,data:['DNS Lookup(ms)', 'Interval(ms)', 'SYN(ms)','Time to First Byte(ms)','Interactive(ms)','FIN(ms)','Delay(ms)']},
        grid: {top:topGr,left: '0%',right: '3%',bottom: '0%',containLabel: true},calculable : true,
        xAxis : [{type : 'value',axisLabel:{formatter:function(val) {return val+"ms";}}}],
...
function use2(title,id,list,x,list_y) {    //折线图堆叠
	...
	option = {
		    ...
		    grid: {top:'26%',left: '1%', right: '2%',bottom: '0%',containLabel: true},
		    toolbox: {feature: { }},
...
function use22(title,id,list,x,list_y) {    //折线图堆叠
	var myChart = echarts.init(document.getElementById(id));
	option = {
			animation:false,
		    title: {text: title,x:'center'},
		    ...
		    grid: {top:'26%',left: '1%', right: '2%',bottom: '0%',containLabel: true},
		    toolbox: {feature: { }},
		    xAxis: {type: 'category',axisLabel: {formatter: '{value}ms'},boundaryGap: false,data: eval(x)},		//  ['周一','周二','周三','周四','周五','周六','周日']
		    yAxis: { type: 'value',axisLabel: {formatter: '{value} Byte'}},series: eval(list_y)// [{name:'status',type:'line',stack: '总量',data:[1,2,1,8]}]
		};
	myChart.setOption(option);
	return myChart;
...
function packetLen(title,id,x,list_y,subtext) {
	var data=eval(list_y);
	//list_y="[{value:972,name:'0-100Byte'},{value:86,name:'101-1000Byte'},{value:1717,name:'1001-1514Byte'}]"
	var myChart = echarts.init(document.getElementById(id));
	option = {
			animation:false,
	...
	return myChart;
								...
								/**图表生成图片 至 服务器*/
function imageEx(imgname,myChart,analysisType){
	var data = (""+imgname+"=" + encodeURIComponent(myChart.getDataURL({type:'png', pixelRatio: 1,backgroundColor: 'white'})));
    $.ajax({type:'post',data: data, dataType:'xml',url : "/MLOAN/img/chart.req?imgname="+imgname+"&analysisType="+analysisType}); }
								/**checkbox*/
