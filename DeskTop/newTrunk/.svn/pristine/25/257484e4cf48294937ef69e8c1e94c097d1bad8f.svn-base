...
<body>
<div class="easyui-panel" title="首页->实物入库管理->物资质检管理->编辑质检单" data-options="fit:true,border:false">
    <form id="formT" method="post" enctype="multipart/form-data">
        <input type="hidden" id="materiaInspectId" name="materiaInspectId" value="${materiaInspectId}"/>
        <input type="hidden" id="materiaInspectStute" name="materiaInspectStute" value="${materiaInspectStuate}"/>
        <%--<input type="hidden" id="supplierId" name="supplierId"/>--%>
        <input type="hidden" id="dataSource_re" name="dataSource_re" value="receipt"/>
        <div style="padding:10px;text-align:-moz-left">
            ...
            <%--//easyui有个双向数据绑定，所以name=“”里面的字符串是硬绑定的--%>
        </div>
        ...
        <a href="javascript:void(0)" iconCls='icon-add' class="easyui-linkbutton" onclick="addDetail()">添加明细</a>
        <a href="javascript:void(0)" iconCls='icon-remove' class="easyui-linkbutton" onclick="delMaterials()">删除明细</a>
    </form>
    <table id="tableM" auto-resize="true" class="easyui-datagrid" style="width:100%;height:200px" title="物资明细" >
    </table>
    <table id="tableT" auto-resize="true" class="easyui-datagrid" style="width:100%;height:200px" title="图片文件展示">
    </table>
    <div style="text-align: center;">
        ...
        <div style="text-align:left;">
            <a href="${emms}/instorage/materialManag.do?cmd=query" iconCls='icon-back' class="easyui-linkbutton" >返回</a>
        </div>
    </div>
    </form>
    <form method="post" style="display: none;" enctype="multipart/form-data" id="fileFrom"></form>
</div>
<script type="text/javascript">
    ...
    $(function(){
        ...
        console.log(materiaInspectStute);
        if(materiaInspectStute == "Checking"){
           $("#supplierId").combobox('disable');
        }else{
            $("#supplierId").combobox('enable');
        }
        $('#formT').form('load', '${emms}/instorage/materialManag.do?cmd=loadMaterialInspectData&materiaInspectId=${materiaInspectId}');
        $('#formT').form({
    ...
    $('#tableM').datagrid({
        ...
        columns:[[
            ...
            {field:'storageCode',hidden:true},
            {field:'deliveryId',hidden:true},//收货单ID
            {field:'packingDetailId',hidden:true},//收货单明细ID
            {field:'receiptCode',sortable:true,title:'收货单编号',align:'center',width:'7%'},
            {field:'packingNo',sortable:true,title:'包装编号',align:'center',width:'6%'},
            {field:'materialsCode',sortable:true,title:'物资编码',align:'center',width:'6%'},
            {field:'materialsDescribe',sortable:true,title:'物资描述',align:'center',width:'6%'},
            {field:'additional1',sortable:true,title:'附加1',align:'center',width:'4%'},
            {field:'additional2',sortable:true,title:'附加2',align:'center',width:'4%'},
            {field:'additional3',sortable:true,title:'附加3',align:'center',width:'4%'},
            {field:'additional4',sortable:true,title:'附加4',align:'center',width:'4%'},
            ...
            {field:'productDate',sortable:true,title:'生产日期',align:'center',width:'10%',required:true,editor:{type:'datebox'}},
            {field:'qualityDate',sortable:true,title:'保质期',align:'center',width:'10%',required:true,editor:{type:'datebox'}},
            {field:'purchaseCount',sortable:true,title:'采购数量',align:'center',width:'6%'
                //editor:{type:'numberbox',options:{required:true}}
            },
            {field:'deliveryQty',sortable:true,title:'已发货数量',align:'center',width:'7%'
               // editor:{type:'numberbox',options:{required:true}}
            },
            {field:'currentDeliveryQty',sortable:true,title:'本次发货数量',align:'center',width:'7%'
                //editor:{type:'numberbox',options:{required:true}}
            },
            {field:'dianshouCount',sortable:true,title:'点收数量',align:'center',width:'6%',
                editor:{type:'numberbox',options:{precision:4}}
            },
            {field:'qualifiedQty',sortable:true,title:'合格数量',align:'center',width:'6%',
               editor:{type:'numberbox',options:{precision:4}},
                formatter: function(value,row,index){
                    $.ajax({
                        type: 'POST',
                        url: "${emms}/instorage/materialManag.do?cmd=CheckOutReceipt",
            ...
            {field:'unQualifiedQty',sortable:true,title:'不合格数量',align:'center',width:'6%',
                editor:{type:'numberbox',options:{precision:4}}
            },
    ...
    function addDetail(){
        ...
        top.$('#dialog').dialog({
            ...
            closed: false,
            cache: true,
            href: '${emms}/instorage/receiptGoods.do?cmd=dialogReceiptGoods&dataSource=receipt&supplierId='+supplierId
        });
    }
    }
    ...
    function ajaxDelete(id) {
        ...
        $.messager.confirm("操作提示", "确定要删除吗？", function (data) {
            if(data){
                $.ajax({
                    type: "GET",
                    url: "${emms}/instorage/materialManag.do?cmd=DeletePic&InspectPicId=" + id,
                    async: false,
                    success: function (result) {
                        $.messager.alert("操作提示","删除成功","info");
                        window.location = "${emms}/instorage/materialManag.do?cmd=edit&materiaInspectId="+materiaInspectId+"&materiaInspectStuate="+materiaInspectStute+"";
                    },
                    error: function () {
   ...
   //接收弹出框数据
    ...
    function save(state){
       var e_dataSource = $("#dataSource_re").attr("value");
       var materiaInspectId = $("#materiaInspectId").val();
       ...
       //var inspectPicFile = $("#tableT").datagrid("getData");
        console.log(e_supplierName);
       var inspect = {
           "materiaInspectId":materiaInspectId,
            ...
            //"inspectPicFileList":inspectPicFile.rows,
...
//            }
        ...
        if(isValid){
            $.ajax({
                type: 'POST',
                url: "${emms}/instorage/materialManag.do?cmd=saveInspect",
    ...
    $("#tableT").datagrid({
        ...
        columns:[[
            {field:'fileName',sortable:true,title:'文件名',align:'center',width:'75%'}
        ]],
        view: detailview,
