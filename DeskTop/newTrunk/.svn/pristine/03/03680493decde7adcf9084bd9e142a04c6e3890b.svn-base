...
@Controller
@RequestMapping(value="/system/dataDictionary.do")
public class DataDictionaryController {
	...
	public String dictionary(ModelMap model){
		model.addAttribute("westFrameUrl", "system/dataDictionary.do?cmd=dictionaryTree");
		model.addAttribute("centerFrameUrl", "system/dataDictionary.do?cmd=query&parentId=0");
	...
	public String dictionaryTree(ModelMap model){
		model.addAttribute("initTreeUrl", "system/dataDictionary.do?cmd=initTree");
		model.addAttribute("navUrl", "system/dataDictionary.do?cmd=query&parentId=");
	...
	public List<Dictionary> initTree() throws Exception{
		List<Dictionary> list = dictionaryServiceImpl.selectAllDictionary("1");
	...
	public PageBean<Dictionary> selectDictionaryByParentId(@RequestParam(value = "page",required = false) Integer start,@RequestParam(value = "rows", required = false) Integer length,
		...
		dictionary.setDictionaryType(1);
	...
	public String dictionaryEdit(@Valid @ModelAttribute("dictionary") Dictionary dictionary, BindingResult errors) throws IOException {
		...
		dictionary.setDictionaryType(1);//默认业务字典
