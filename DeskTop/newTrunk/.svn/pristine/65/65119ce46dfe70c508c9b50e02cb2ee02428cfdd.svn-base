...
<body>
	<div class="easyui-panel" title="首页->基础信息管理->仓库管理->储位编辑" data-options="fit:true,border:false">
		<form id="ff" method="post">
			...
			<div style="padding:10px">
				<input class="easyui-textbox" id="storagelocationCode" name="storagelocationCode"  style="width:40%;height:100px" data-options="label:'储位编码:',multiline:true,required:true" >
			 ...
			 $('#ff').form({
					onLoadSuccess:function(data){
						if(null != data.storagelocationId){
							$("#storagelocationCode").textbox({readonly:true});
	...
	function ajaxSubmitForm() {
		  $("#ff").form("submit", {
             url: "${emms}/baseinfo/storagelocation.do?cmd=saveBatchStoragelocation",
             onsubmit: function () {
