...
<body>
<div class="easyui-panel" title="首页->实物入库管理->物资质检管理->物资质检单列表" data-options="fit:true,border:false">
...
<script type="text/javascript">
    ...
    function query() {
        ...
        $('#table').datagrid({
            url: '${emms}/instorage/materialManag.do?cmd=loadMaterialInspectListData',
            method: 'POST',
            pagination: true,
            fitColumns: true,
            rownumbers: true,
            ...
            columns: [[

                {
                    ...
                    formatter: function (value, row, index) {
                        return '<a class="easyui-linkbutton" style="color:blue" href="${emms}/instorage/materialManag.do?cmd=dialogMaterialInspect&materiaInspectId=' + row.materiaInspectId + '" target="_self">' + row.inspectNo + '</a>';
                    }
                },
                ...
                {field: 'inspectStaus', sortable: true, title: '单据状态', align: 'center', width: '15%'},
                {
                    ...
                    formatter: function (value, row) {
                        ...
                        if (row.inspectStaus == '正在质检') {
                            show += "<a class='easyui-linkbutton' href='${emms}/instorage/materialManag.do?cmd=edit&materiaInspectStuate=Checking&materiaInspectId="
                        ...
                        if (row.inspectStaus == '质检完成') {
                            show += "<a class='easyui-linkbutton'  href='${emms}/instorage/materialManag.do?cmd=dialogMaterialInspect&materiaInspectId="
                            + row.materiaInspectId
                            + "' target='_self'>查看</a>&nbsp;&nbsp;&nbsp;";
                        }
                        return show;
    ...
    function ajaxDelete(id) {
        $.messager.confirm('确认对话框', '确定要删除当前记录吗？', function (result) {
            if (result) {
                $.ajax({
                    type: "POST",
                    url: "${emms}/instorage/materialManag.do?cmd=delete&materiaInspectId=" + id,
                    async: false,
                    success: function () {
