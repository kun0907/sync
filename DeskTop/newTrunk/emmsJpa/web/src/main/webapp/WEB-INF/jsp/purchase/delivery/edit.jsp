...
<html>
		...
		<div class="easyui-layout" style="width:100%;height: 700px;">
			<div data-options="region:'west',split:true" style="width:50%;">
				<div class="easyui-panel" title="包装发货明细" data-options="fit:true,border:false">
					<a href="javascript:void(0)" iconCls='icon-add' class="easyui-linkbutton" onclick="append()">增加</a>
					<a href="javascript:void(0)" iconCls='icon-remove' class="easyui-linkbutton" onclick="removeIt()">移除</a>
					<table title="包装明细" class="easyui-treegrid" id="table"></table>
				</div>
			</div>
			<div data-options="region:'center'" title="包装物料列表" style="width:50%">
				<a href="javascript:void(0)" iconCls='icon-add' class="easyui-linkbutton" onclick="newMaterials()">新增物料</a>
				<a href="javascript:void(0)" iconCls='icon-remove' class="easyui-linkbutton" onclick="delMaterials()">删除物料</a>
			...
			<div data-options="region:'south',split:true" style="width:100%;height: 500px">
				<div class="easyui-panel" title="运输明细" data-options="fit:true,border:false">
					...
					<a href="javascript:void(0)" iconCls='icon-remove' class="easyui-linkbutton" onclick="delVehicleInfo()">删除运输明细</a>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('#table').treegrid({
				...
				onClickRow:function(row){
					$('#detail').datagrid('endEdit',0);
					if(lastDetail !=''){
						$('#detail').datagrid('endEdit', lastDetail);
					}
					$('#table').treegrid('endEdit', lastPackage);
					if(lastPackage != row.id){
						var lastDetailList = $("#detail").datagrid("getRows");
						if(lastPackage != ''){
							for(var i=0;i<lastDetailList.length;i++){
								if(null == lastDetailList[i].deliveryMainCount || lastDetailList[i].deliveryMainCount == '' || parseFloat(lastDetailList[i].deliveryMainCount) == 0 ){
									$.messager.alert("操作提示", "明细中预发货数量必须大于0","warning");
									$('#table').treegrid('select',lastPackage);
									return ;
								}
								packageDetail.push(lastDetailList[i]);
							}
						}
						var checkDetail = [];
			...
			$('#detail').datagrid({
				...
				singleSelect:true,
				onDblClickRow:function(row){//运用双击事件实现对一行的编辑
					if(null == $('#table').treegrid('getSelected')){
						$.messager.alert("操作提示", "请选择包装后再编辑明细","warning");
						return ;
					}else{
			...
			$('#vehicle').datagrid({
				...
				onClickRow:function(index,row) {//运用单击事件实现一行的编辑结束，在该事件触发前会先执行onAfterEdit事件
					$('#vehicle').datagrid('endEdit',0);
					if(lastVehicle !=''){
						$('#vehicle').datagrid('endEdit', lastVehicle);
					}
					lastVehicle = index;
				},
				columns: [[
		...
		function append(){
			...
			if(null == node){
				$.messager.alert("操作提示", "请选择需要在哪个节点创建子节点！","warning");
				return ;
			}
		...
		function removeIt(){
			var node = $('#table').treegrid('getSelected');
			if(null == node){
				$.messager.alert("操作提示", "请选择需要删除的节点！","warning");
				return ;
			}
			if(node.id == '0'){
		...
		function ajaxSubmitForm() {
			if(null != $("#table").treegrid("getSelections").length>0){
				$('#table').treegrid('endEdit', $("#table").treegrid("getSelected").id);
			}
			$('#detail').datagrid('endEdit', $("#detail").datagrid("getRowIndex"));
			$('#vehicle').datagrid('endEdit', $("#vehicle").datagrid("getRowIndex"));
			//获取包装数据并转换为json
			var packageInfo = [];
			...
			for(var i=0;i<package.length;i++){
				if(package[i].id !='0'){
					if (null == package[i].packingNo || package[i].packingNo == '') {
						$.messager.alert("操作提示", "包装编号不能为空", "warning");
						return false;
					}
					if (null == package[i].packingType || package[i].packingType == '') {
						$.messager.alert("操作提示", "包装形式不能为空", "warning");
						return false;
					}
				}
